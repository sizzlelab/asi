...
#  before_filter :ensure_person_logout, :only  => [ :create, :recover_password ]
  #changed by marcos to test how to create a user with default profile rule
  before_filter :ensure_person_logout, :only  => [:recover_password ]
...
=end
  def create
    ...
    if @person.save
      ...
      #added by marcos to assign a default profile rule to the user by the time he is created
      Rule.create_default_profile_rule(@person)
