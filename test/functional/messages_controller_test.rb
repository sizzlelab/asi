...
class MessagesControllerTest < ActionController::TestCase
  ...
  def test_list_messages
    ...
    assert_equal 5, json["entry"].length

    # index without replies
    get :index, {:format => "json", :channel_id => channels(:julkikanava).guid, :exclude_replies => true }, {:cos_session_id => sessions(:session1).id}
    assert_response :ok, @response.body
    json = JSON.parse(@response.body)
  ...
  def test_show_replies_to_message
    get :replies, {:format => "json", :channel_id => channels(:julkikanava).guid, :msg_id => messages(:testiviesti2).guid }, {:cos_session_id => sessions(:session1).id}
    assert_response :ok, @response.body
    json = JSON.parse(@response.body)
    assert_equal 2, json["entry"].length
    assert_equal messages(:replyto_testiviesti2_1).body, json["entry"][0]["body"]
  end
